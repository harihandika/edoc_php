 ...
 * @version    Release: @package_version@
 */
class SeedDMS_View_ViewDocumentReq extends SeedDMS_Bootstrap_Style {
	...
	function js() { /* {{{ */
		...
		if($user->isAdmin()) {
			...
			$this->printTimelineJs('out.ViewDocumentReq.php?action=timelinedata&documentid='.$latestContent->getDocument()->getID(), 300, '', date('Y-m-d'));
		}
...
<?php
	...
	function show() { /* {{{ */
		...
		$this->contentStart();
		$this->pageNavigation(null, "view_document", $document);
...
//		print "<li class=\"wordbreak\">".$latestContent->getOriginalFileName() ."</li>\n";
		...
		if ($file_exists){
			...
			if($accessobject->check_controller_access('Download', array('action'=>'version')) && $status["status"]!=S_OBSOLETE) {
