...
<?php
			...
			if(!in_array($seluser->getID(), $undeluserids) && $accessobject->check_view_access('RemoveUser')) {
				$menuitems['removeuser'] = array('link'=>$this->html_url('RemoveUser', array('userid'=>$seluser->getID())), 'label'=>'<i class="fa fa-remove"></i> '.getMLText("rm_user"));
			}
			$menuitems['removeuserfromprozess'] = array('link'=>$this->html_url('RemoveUserFromProcesses', array('userid'=>$seluser->getID())), 'label'=>'<i class="fa fa-unlink"></i> '.getMLText("rm_user_from_processes"));
			$menuitems['transferobjects'] = array('link'=>$this->html_url('TransferObjects', array('userid'=>$seluser->getID())), 'label'=>'<i class="fa fa-share-square-o"></i> '.getMLText("transfer_objects"));
			if($user->isAdmin() && $seluser->getID() != $user->getID())
				$menuitems['substituteuser'] = array('link'=>'../op/op.SubstituteUser.php?userid='.$seluser->getID().'&formtoken='.createFormKey('substituteuser'), 'label'=>'<i class="fa fa-exchange"></i> '.getMLText("substitute_user"));
			if($enableemail)
				$menuitems['sendlogindata'] = array('link'=>$this->html_url('SendLoginData',array('userid'=>$seluser->getID())), 'label'=>'<i class="fa fa-envelope-o"></i> '.getMLText("send_login_data"));
