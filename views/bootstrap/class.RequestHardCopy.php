 ...
 * @version    Release: @package_version@
 */
class SeedDMS_View_RequestHardCopy extends SeedDMS_Bootstrap_Style {
...
<form class="form-horizontal" action="../op/op.RequestHardCopy.php" id="form1" name="form1" method="post">
	<?php echo createHiddenFieldWithKey('requesthardcopy'); ?>
...
foreach($documents as $document) {
	$options[] = array(htmlspecialchars($document->getID()), htmlspecialchars($document->getName()) . '   --    ' . htmlspecialchars($document->getOwner()->getFullName()));
		...
		$this->formField(
			getMLText("tanggal_pinjam"),
			$this->getDateChooser('', "date", $this->params['session']->getLanguage())
		);
		...
		$this->formSubmit("<i class=\"fa fa-save\"></i> ".getMLText('request_hard_copy'));
?>
</form>
<?php
		// $this->contentContainerEnd();
