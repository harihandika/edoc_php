...
if ($action == "adduser") {
	...
	if($userrole->getRole() == 0){
		$params['role'] = 'User';
	}else if($userrole->getRole() == 1){
		$params['role'] = 'Admin';
	}else if($userrole->getRole() == 2){
		$params['role'] = 'Guest';
	}
	...
	$notifier->toIndividual($user, $email, $subject, $message, $params, SeedDMS_NotificationService::RECV_OWNER);
...
// send login data to user
	...
	if($notifier) {
		...
		$params['password'] = $newuser->getPwd();
		$params['date'] = date("d.m.Y");
		$role = $newuser->getRole();
		$userrole = $newuser->getRole()->getRole();
		if($userrole == 0){
			$params['role'] = 'User';
		}else if($userrole == 1){
			$params['role'] = 'Admin';
