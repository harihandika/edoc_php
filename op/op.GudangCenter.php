...
if ($accessop->check_controller_access($controller, array('action'=>'setOwner'))) {
	...
	if($ownerid) {
		if(!($owner = $dms->getUser($ownerid))) {
			UI::exitError(getMLText("folder_title", array("foldername" => $gudangcenter->getName())),getMLText("error_occured"));
		}
	} else {
		$owner = $user;
	}
...
$docsource = 'upload';
	...
	if(!$settings->_enableDuplicateDocNames) {
		if($gudangcenter->hasDocumentByName($name)) {
			UI::exitError(getMLText("folder_title", array("foldername" => $gudangcenter->getName())),getMLText("document_duplicate_name"));
		}
	}
	...
	if(!$gudangcenter = $controller->run()) {
		...
		if($notifier) {
			...
			if($workflow && $settings->_enableNotificationWorkflow) {
				...
				$params = array();
				$params['name'] = $gudangcenter->getName();
	...
	add_log_line("?name=".$name."&gudangcenterid=".$gudangcenterid);
	// }

header("Location:../out/out.ViewFolder.php?gudangcenterid=".$gudangcenterid."&showtree=".$_POST["showtree"]);
