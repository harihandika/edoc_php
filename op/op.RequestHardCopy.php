...
if(!checkFormKey('requesthardcopy')) {
...
if ($accessop->check_controller_access($controller, array('action'=>'setOwner'))) {
	...
	if($ownerid) {
		if(!($owner = $dms->getUser($ownerid))) {
			UI::exitError(getMLText("folder_title", array("foldername" => $requesthardcopy->getName())),getMLText("error_occured"));
		}
	} else {
		$owner = $user;
	}
...
$docsource = 'upload';
	...
	if(!$settings->_enableDuplicateDocNames) {
		if($requesthardcopy->hasDocumentByName($name)) {
			UI::exitError(getMLText("folder_title", array("foldername" => $requesthardcopy->getName())),getMLText("document_duplicate_name"));
		}
	}
	...
	if(!$requesthardcopy = $controller->run()) {
		...
		if($notifier) {
			...
			if($workflow && $settings->_enableNotificationWorkflow) {
				...
				$params = array();
				$params['name'] = $requesthardcopy->getName();
	...
	add_log_line("?name=".$name."&requesthardcopyid=".$requesthardcopyid);
	// }

header("Location:../out/out.ViewFolder.php?requesthardcopyid=".$requesthardcopyid."&showtree=".$_POST["showtree"]);
