...
//		$queryStr = "SELECT * FROM `tblDocuments` WHERE `id` = " . (int) $id;
	...
	function customGetLatestContent() { /* {{{ */
		if (!$this->_latestContent) {
			$db = $this->_dms->getDB();
			$queryStr = "SELECT dsl.status status, gdp.name AS codeProject,  gld.document as idDocument, gld.version,
			gd.folder idFolder ,gd.folderName as category, gd.name, gld.orgFileName, gld.date 
            FROM tblDocumentContent gld 
			JOIN getdocdata gd 
				ON gld.document = gd.id 
			LEFT JOIN getdocumentparent gdp 
				ON gd.parentId = gdp.id
			LEFT JOIN tbldocumentstatus ds
				ON ds.documentID = gd.id
			LEFT JOIN tbldocumentstatuslog dsl
				ON ds.statusID = dsl.statusID
				WHERE gld.document = " . $this->_id . "
                ORDER BY `version` DESC, dsl.statusLogID DESC
            LIMIT 1";
			...
			foreach ($resArr as $row) {
				/** @var SeedDMS_Core_DocumentContent $content */
				if (!$this->_latestContent) {
					$content = array('status' => $row["status"], 'codeProject' => $row["codeProject"], 'idDocument' => $row["idDocument"], 'version' => $row["version"], 'idFolder' => $row["idFolder"], 'category' => $row["category"], 'name' => $row["name"], 'orgFileName' => $row["orgFileName"], 'date' => $row['date']);
					if($user) {
						/* If the user may even write the document, then also allow to see all content.
						 * This is needed because the user could upload a new version
						 */
