...
//				$this->_subFolders[$i] = $this->_dms->getFolder($resArr[$i]["id"]);
	...
	function getCodeProject($id){
		...
		// var_dump($id);
		// die();
		$queryStr = "SELECT gdp.parentName AS projectName FROM getdocdata as gd LEFT JOIN getdocumentparent gdp ON gd.parentId = gdp.id WHERE gd.folder =". $id;
		$resArr = $db->getResultArray($queryStr);

		if (is_bool($resArr) && !$resArr)
			return false;

		return $resArr[0]['projectName'];
	}
...
//				array_push($this->_documents, $this->_dms->getDocument($row["id"]));
	...
	function getViewSDMS($codeProject, $orderby="", $dir="asc", $limit=0, $offset=0){
		$db = $this->_dms->getDB();

		if (!isset($this->_documents)) {

			$queryStr = "SELECT gdp.name AS codeProject, gd.* , dl.userID as `lock`
			FROM getdocdata as gd
			LEFT JOIN getdocumentparent gdp
				ON gd.parentId = gdp.id
				LEFT JOIN tblDocumentLocks dl ON gd.id = dl.document
			WHERE gdp.name = '" . $codeProject . "'
			ORDER BY codeProject";
