 ...
 * @version    Release: 6.0.15
 */
class SeedDMS_Core_WorkLocation { /* {{{ */
	/**
	 ...
	 * @return SeedDMS_Core_WorkLocation|bool instance of class SeedDMS_Core_Group if group was
	...
	public static function getInstance($id, $dms, $by='') { /* {{{ */
		...
		switch($by) {
		case 'name':
			$queryStr = "SELECT * FROM `tblWorkLocations` WHERE `name` = ".$db->qstr($id);
			break;
		default:
			$queryStr = "SELECT * FROM `tblWorkLocations` WHERE `id` = " . (int) $id;
		}
		...
		$worklocation = new self($resArr["id"], $resArr["name"], $resArr["comment"]);
		$worklocation->setDMS($dms);
		return $worklocation;
	} /* }}} */
	...
	public static function getAllInstances($orderby, $dms) { /* {{{ */
		...
		switch($orderby) {
		default:
			$queryStr = "SELECT * FROM `tblWorkLocations` ORDER BY `name`";
		}
		$resArr = $db->getResultArray($queryStr);
		...
		for ($i = 0; $i < count($resArr); $i++) {
			$worklocation = new self($resArr[$i]["id"], $resArr[$i]["name"], $resArr[$i]["comment"]);
			$worklocation->setDMS($dms);
			$worklocations[$i] = $worklocation;
		}
	...
	function setName($newName) { /* {{{ */
		...
		$queryStr = "UPDATE `tblWorkLocations` SET `name` = ".$db->qstr($newName)." WHERE `id` = " . $this->_id;
	...
	function setComment($newComment) { /* {{{ */
		...
		$queryStr = "UPDATE `tblWorkLocations` SET `comment` = ".$db->qstr($newComment)." WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	function getUsers() { /* {{{ */
		...
		if (!isset($this->_users)) {
			$queryStr = "SELECT `tblUsers`.* FROM `tblUsers` ".
				"LEFT JOIN `tblWorkMembers` ON `tblWorkMembers`.`userID`=`tblUsers`.`id` ".
				"WHERE `tblWorkMembers`.`worklocationID` = '". $this->_id ."'";
			$resArr = $db->getResultArray($queryStr);
			if (is_bool($resArr) && $resArr == false)
				return false;
	...
	function getManagers() { /* {{{ */
		...
		$queryStr = "SELECT `tblUsers`.* FROM `tblUsers` ".
			"LEFT JOIN `tblWorkMembers` ON `tblWorkMembers`.`userID`=`tblUsers`.`id` ".
			"WHERE `tblWorkMembers`.`worklocationID` = '". $this->_id ."' AND `tblWorkMembers`.`manager` = 1";
		$resArr = $db->getResultArray($queryStr);
		if (is_bool($resArr) && $resArr == false)
			return false;
	...
	function addUser($user,$asManager=false) { /* {{{ */
		...
		$queryStr = "INSERT INTO `tblWorkMembers` (`worklocationID`, `userID`, `manager`) VALUES (".$this->_id.", ".$user->getID(). ", " . ($asManager?"1":"0") ." )";
	...
	function removeUser($user) { /* {{{ */
		...
		$queryStr = "DELETE FROM `tblWorkMembers` WHERE `worklocationID` = ".$this->_id." AND `userID` = ".$user->getID();
		$res = $db->getResult($queryStr);
	...
	function isMember($user,$asManager=false) { /* {{{ */
		...
		$db = $this->_dms->getDB();
		if ($asManager) $queryStr = "SELECT * FROM `tblWorkMembers` WHERE `worklocationID` = " . $this->_id . " AND `userID` = " . $user->getID() . " AND `manager` = 1";
		else $queryStr = "SELECT * FROM `tblWorkMembers` WHERE `worklocationID` = " . $this->_id . " AND `userID` = " . $user->getID();
	...
	function toggleManager($user) { /* {{{ */
		...
		if ($this->isMember($user,true)) $queryStr = "UPDATE `tblWorkMembers` SET `manager` = 0 WHERE `worklocationID` = ".$this->_id." AND `userID` = ".$user->getID();
		else $queryStr = "UPDATE `tblWorkMembers` SET `manager` = 1 WHERE `worklocationID` = ".$this->_id." AND `userID` = ".$user->getID();
	...
	function remove($user) { /* {{{ */
		...
		$queryStr = "DELETE FROM `tblWorkMembers` WHERE `worklocationID` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
		$queryStr = "DELETE FROM `tblACLs` WHERE `worklocationID` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
		$queryStr = "DELETE FROM `tblNotify` WHERE `worklocationID` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
		$queryStr = "DELETE FROM `tblMandatoryReviewers` WHERE `reviewerWorkLocationID` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
		$queryStr = "DELETE FROM `tblMandatoryApprovers` WHERE `approverWorkLocationID` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
		$queryStr = "DELETE FROM `tblWorkflowTransitionWorkLocations` WHERE `worklocationid` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
		$queryStr = "DELETE FROM `tblWorkLocations` WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
		...
		foreach ($reviewStatus as $r) {
			$queryStr = "INSERT INTO `tblDocumentReviewLog` (`reviewID`, `status`, `comment`, `date`, `userID`) ".
				"VALUES ('". $r["reviewID"] ."', '-2', 'Review work location removed from process', ".$db->getCurrentDatetime().", '". $user->getID() ."')";
			$res=$db->getResult($queryStr);
			if(!$res) {
				$db->rollbackTransaction();
				return false;
		...
		foreach ($approvalStatus as $a) {
			$queryStr = "INSERT INTO `tblDocumentApproveLog` (`approveID`, `status`, `comment`, `date`, `userID`) ".
				"VALUES ('". $a["approveID"] ."', '-2', 'Approval work location removed from process', ".$db->getCurrentDatetime().", '". $user->getID() ."')";
			$res=$db->getResult($queryStr);
			if(!$res) {
				$db->rollbackTransaction();
				return false;
		...
		foreach ($receiptStatus as $r) {
			$queryStr = "INSERT INTO `tblDocumentReceiptLog` (`receiptID`, `status`, `comment`, `date`, `userID`) ".
				"VALUES ('". $r["receiptID"] ."', '-2', 'Recipients work location removed from process', ".$db->getCurrentDatetime().", '". $user->getID() ."')";
			$res=$db->getResult($queryStr);
			if(!$res) {
				$db->rollbackTransaction();
				return false;
		...
		foreach ($revisionStatus as $r) {
			$queryStr = "INSERT INTO `tblDocumentRevisionLog` (`revisionID`, `status`, `comment`, `date`, `userID`) ".
				"VALUES ('". $r["revisionID"] ."', '-2', 'Revisors work location removed from process', ".$db->getCurrentDatetime().", '". $user->getID() ."')";
			$res=$db->getResult($queryStr);
			if(!$res) {
				$db->rollbackTransaction();
				return false;
	...
	function getReviewStatus($documentID=null, $version=null) { /* {{{ */
		...
		// See if the work location is assigned as a reviewer.
	...
	function getWorkflowStatus($documentID=null, $version=null) { /* {{{ */
		...
		$queryStr = 'select distinct d.*, c.`worklocationid` from `tblWorkflowTransitions` a left join `tblWorkflows` b on a.`workflow`=b.`id` left join `tblWorkflowTransitionGroups` c on a.`id`=c.`transition` left join `tblWorkflowDocumentContent` d on b.`id`=d.`workflow` where d.`document` is not null and a.`state`=d.`state` and c.`worklocationid`='.$this->_id;
		if($documentID) {
	...
	function getNotifications($type=0) { /* {{{ */
		...
		$queryStr = "SELECT `tblNotify`.* FROM `tblNotify` ".
		 "WHERE `tblNotify`.`worklocationID` = ". $this->_id;
		if($type) {
		...
		$notifications = array();
		foreach ($resArr as $row) {
			$not = new SeedDMS_Core_Notification($row["target"], $row["targetType"], $row["userID"], $row["worklocationID"]);
