 ...
 * @version    Release: 6.0.15
 */
class SeedDMS_Core_RequestHardCopy extends SeedDMS_Core_Object { /* {{{ */
	/**
	 ...
	 * @return SeedDMS_Core_RequestHardCopy|bool instance of SeedDMS_Core_HardCOpy if document exists
	 */
	public static function getInstanceByData($resArr, $dms) { /* {{{ */
		$classname = $dms->getClassname('hardcopy');
		/** @var SeedDMS_Core_HardCopy $hardcopy */
		$hardcopy = new self($resArr["id"], $resArr["documentID"], $resArr["keterangan"], $resArr["keperluan"], $resArr["date"],$resArr["owner"], $resArr["inheritAccess"], $resArr["defaultAccess"], $resArr["status"],$resArr["expires"], $resArr["documentlocation"],$resArr["origin"],$resArr["destiny"]);
		$hardcopy->setDMS($dms);
		$hardcopy = $hardcopy->applyDecorators();
		return $hardcopy;
	} /* }}} */
	 ...
	 * @param SeedDMS_Core_DMS $dms
	 * @return SeedDMS_Core_HardCopy|bool instance of SeedDMS_Core_HardCOpy if document exists, null
	 * if document does not exist, false in case of error
	 */
	public static function getInstance($id, $dms) { /* {{{ */
		$db = $dms->getDB();
		$queryStr = "SELECT * FROM `tblRequestHardCopy` WHERE `id` = " . (int) $id;
	...
	public static function getAllInstances($orderby, $dms) { /* {{{ */
		...
		$queryStr = "SELECT * FROM `tblRequestHardCopy` ORDER BY `id` DESC";
		$resArr = $db->getResultArray($queryStr);
		if (is_bool($resArr) && $resArr == false){
			return false;
		}

			$requesthardcopys = array();
			for ($i = 0; $i < count($resArr); $i++) {
				$requesthardcopy = new self($resArr[$i]["id"], $resArr[$i]["documentID"], $resArr[$i]["keterangan"],$resArr[$i]["keperluan"],$resArr[$i]["date"],$resArr[$i]["owner"],$resArr[$i]["inheritAccess"],$resArr[$i]["defaultAccess"],$resArr[$i]["status"],$resArr[$i]["expires"],$resArr[$i]["documentlocation"],$resArr[$i]["origin"],$resArr[$i]["destiny"]);
				$requesthardcopy->setDMS($dms);
				$requesthardcopys[$i] = $requesthardcopy;
			}
	
			return $requesthardcopys;
	} /* }}} */
	...
	public function setDocumentID($newDocumentID) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `documentID` = " . $db->qstr($newDocumentID) . " WHERE `id` = ". $this->_id;
	...
	public function setKeterangan($newKeterangan) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `keterangan` = " . $db->qstr($newKeterangan) . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	public function setKeperluan($newKeperluan) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `keperluan` = " . $db->qstr($newKeperluan) . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	public function setDocumentLocation($newDocumentLocation) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `documentlocation` = " . $db->qstr($newDocumentLocation) . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	public function setOrigin($newOrigin) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `origin` = " . $db->qstr($newOrigin) . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	public function setDestiny($newDestiny) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `destiny` = " . $db->qstr($newDestiny) . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	public function setStatus($newStatus) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `status` = " . $db->qstr($newStatus) . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	function setDate($date) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `date` = " . (int) $date . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
		$this->_date = $date;
		return true;
	...
	function setOwner($newOwner) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` set `owner` = " . $newOwner->getID() . " WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	function setDefaultAccess($mode, $noclean=false) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` set `defaultAccess` = " . (int) $mode . " WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	function setInheritAccess($inheritAccess, $noclean=false) { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `inheritAccess` = " . (int) $inheritAccess . " WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	function getNotifyList($type=0, $incdisabled=false) { /* {{{ */
		if (empty($this->_notifyList)) {
			...
			$queryStr ="SELECT * FROM `tblNotify` WHERE `targetType` = " . T_REQUESTHARDCOPY . " AND `target` = " . $this->_id;
			$resArr = $db->getResultArray($queryStr);
			if (is_bool($resArr) && $resArr == false)
				return false;
	...
	function addNotify($userOrGroupID, $isUser) { /* {{{ */
		...
		*/
		...
		$queryStr = "SELECT * FROM `tblNotify` WHERE `tblNotify`.`target` = '".$this->_id."' ".
			"AND `tblNotify`.`targetType` = '".T_REQUESTHARDCOPY."' ".
		...
		$queryStr = "INSERT INTO `tblNotify` (`target`, `targetType`, " . $userOrGroup . ") VALUES (" . $this->_id . ", " . T_REQUESTHARDCOPY . ", " .  (int) $userOrGroupID . ")";
		if (!$db->getResult($queryStr))
		return -4;
	...
	function getAccessList($mode = M_ANY, $op = O_EQ) { /* {{{ */
		...
		if (!isset($this->_accessList[$mode])) {
			...
			$queryStr = "SELECT * FROM `tblACLs` WHERE `targetType` = ".T_REQUESTHARDCOPY.
...
// *********************************
	...
	function receiveNotify() { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `status` = ". 1 ." WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
		return true;
	}
	...
	function approveNotify() { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `status` = ". 2 .", `documentlocation` = `destiny` WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
		return true;
	} /* }}} */
	...
	function rejectNotify() { /* {{{ */
		...
		$queryStr = "UPDATE `tblRequestHardCopy` SET `status` = ". -2 ." WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
		return true;
	} /* }}} */
	...
	function removeNotify() { /* {{{ */
		...
		$queryStr = "DELETE FROM `tblRequestHardCopy` WHERE `id` = " . $this->_id;
